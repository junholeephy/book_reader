...
Nielsen & Chuang, *Quantum Computation and Quantum Information* 을 읽기 위한 개인 학습 환경.
...
4. KaTeX 내려받아 벤더링
5. 답변 워커를 `~/.qc-book-tutor/` 에 설치

**쪽번호 오프셋**은 지면에 인쇄된 번호를 읽어 알아냅니다.
N&C 로 검증했을 때 표본 8개가 만장일치로 34를 가리켰습니다.
...
~/.qc-book-tutor/           설치된 워커 — 의도적으로 이 트리 바깥
```

### 워커가 프로젝트 밖에 있는 이유

Claude Code는 `CLAUDE.md` 를 **상위 디렉터리로 거슬러 올라가며** 찾습니다.
워커를 `qc_book/` 안에 두면 책에 대한 질문 하나마다 AI-DLC 개발 규칙 전체가 로드됩니다.
